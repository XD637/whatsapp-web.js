...
client.on("message", async (message) => {
    try {
        ...
        if (message.hasQuotedMsg) {
            ...
            let replyContact;
            try {
                replyContact = await quotedMsg.getContact();
            } catch (e) {
                replyContact = null;
            }
            replyTo = {
                To: (replyContact && (replyContact.pushname || replyContact.verifiedName || replyContact.number)) || quotedMsg.author || quotedMsg.from,
        ...
        for (let i = 0; i < chatList.length; i++) {
            ...
            if (
                ...
                if (!fs.existsSync(msgMedia)) {
                    console.error(
                        `Media file does not exist at path: ${msgMedia}`
                    );
                ...
                try {
                    ...
                    console.log(
                        `Message sent to group "${grpName}": ${msgText}`
                    );
                    return "Message sent successfully";
                } catch (sendErr) {
                    console.error(
                        `Failed to send message to "${grpName}":`,
                        sendErr
                    );
...
app.post("/api/send-message", async (req, res) => {
    ...
    if (!groupName || !messageText) {
        return res
            .status(400)
            .json({ error: "groupName and messageText are required" });
    }
...
app.post("/ai/sendmessage", apiLimiter, async (req, res) => {
    try {
        if (!req.body.hasOwnProperty("aiinference")) {
            return res.json({
                code: false,
                message: "AI inference id missing.",
                error: null,
            });
        }
        try {
            ...
            await sendApiMessage(
                "testing",
                `ERROR OCCURED AT AI INFERENCE - ${req.body.aiinference}`,
                imagePath
            );
            return res.json({
                code: true,
...
app.post("/api/create-group", async (req, res) => {
    ...
    if (!groupName || !participants || !Array.isArray(participants)) {
        return res
            .status(400)
            .json({ error: "groupName and participants (array) are required" });
    }
...
app.post("/api/add-members", async (req, res) => {
    ...
    if ((!groupId && !groupName) || !members || !Array.isArray(members)) {
        return res
            .status(400)
            .json({
                error: "groupId or groupName and members (array) are required",
            });
    }
...
app.post("/api/manage-admins", async (req, res) => {
    ...
    if (
        ...
        return res
            .status(400)
            .json({
                error: "groupId or groupName, members (array), and action (promote/demote) are required",
            });
    }
    ...
    try {
        ...
        if (action === "promote") {
            ...
            res.json({
                success: true,
                message: "Members promoted to admin successfully",
            });
        } else if (action === "demote") {
            await demoteFromAdmin(groupId, members);
            res.json({
                success: true,
                message: "Members demoted from admin successfully",
            });
        }
    } catch (error) {
...
app.post("/api/send-media-message", async (req, res) => {
    ...
    if ((!groupId && !groupName) || !message || !mediaPath) {
        return res
            .status(400)
            .json({
                error: "groupId or groupName, message, and mediaPath are required",
            });
    }
...
app.post("/api/update-group-picture", async (req, res) => {
    ...
    if ((!groupId && !groupName) || !imagePath) {
        return res
            .status(400)
            .json({ error: "groupId or groupName and imagePath are required" });
    }
    ...
    try {
        ...
        if (!fs.existsSync(imagePath)) {
            return res
                .status(400)
                .json({
                    error: "Image file does not exist at the specified path",
                });
        }
        ...
        res.status(500).json({
            error: "Failed to update group profile picture",
        });
    }
});
        ...
        if (waCmd === "SendMessage") {
            try {
                ...
                if (await isValidPhoneNumber(recipient)) {
                    result = await sendMessageToNumber(
                        recipient,
                        message,
                        media
                    );
            ...
            console.log(
                "Chat List => " + sock.remoteAddress + ": " + chatListStr
            );
            sockets.forEach(function (sock) {
...
async function broadcastMessageToClients(message) {
    // message is already in the correct format, just stringify and send
        ...
        if (notAdded.length > 0) {
            ...
            for (const restrictedNumber of notAdded) {
                try {
                    ...
                    console.error(
                        `Failed to send invite link to ${restrictedNumber}:`,
                        error.message
                    );
                }
            }
...
async function addMembersToGroup(groupId, members) {
    try {
        ...
        for (const [participant, info] of Object.entries(result)) {
            if (info.code !== 200) {
                console.warn(
                    `Could not add member ${participant}: code ${info.code}`
                );
        ...
        return {
            added: toAdd.length - restrictedNumbers.length,
            invited: restrictedNumbers.length,
        };
    } catch (error) {
